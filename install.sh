...
echo "Would you like to create a new ABM config now?"
  if [[ -a $installpath/Ascii-Bukkit-Menu/include/config ]]; then
    echo
    echo "You already have a config, however it has been backed up."
    echo "Creating a new config will overwrite your current."
    echo "However if you choose to revert, there is copy of your original here:"
    echo
